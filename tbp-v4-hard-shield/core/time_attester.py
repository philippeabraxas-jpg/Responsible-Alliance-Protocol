    ...
    def from_dict(cls, d: Dict[str, Any]) -> "TimestampToken":
        ...
        if d.get("tsa_certificate"):
            cert = x509.load_pem_x509_certificate(
                d["tsa_certificate"].encode("ascii"), default_backend()
            )
...
class TimeAttester:
    ...
    def _start_monitoring(self):
                ...
                try:
                    for server in self.tsa_servers:
                        if server["url"] == "mock":
                            continue
                        try:
                            response = requests.head(server["url"], timeout=5, allow_redirects=True)
        ...
        self._monitor_thread = threading.Thread(
            target=monitor_loop, daemon=True, name="TSA-Monitor"
        )
    ...
    def _build_tsa_request(self, data_hash: bytes, nonce: int) -> bytes:
        ...
        hash_algo = algos.DigestAlgorithm({"algorithm": hash_oid})

        message_imprint = tsp.MessageImprint(
            {"hash_algorithm": hash_algo, "hashed_message": data_hash}
        )
    ...
    def _parse_tsa_response(
        self, response_data: bytes, tsa_url: str, expected_nonce: int
    ) -> TimestampToken:
        """
        ...
        try:
            ...
            if status != "granted":
                fail_info = (
                    status_info["fail_info"].native if status_info["fail_info"] else "unknown"
                )
                status_string = (
                    status_info["status_string"].native if status_info["status_string"] else ""
                )
            ...
            serial_number = tst_info["serial_number"].native.to_bytes(
                (tst_info["serial_number"].native.bit_length() + 7) // 8, "big"
            )
...
if __name__ == "__main__":
    ...
    if ASN1_AVAILABLE:
        ...
        try:
            attester = TimeAttester(tsa_type=TSAType.FREETSA, timeout=20)

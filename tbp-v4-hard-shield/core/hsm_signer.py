...
class HSMSigner:
    ...
    def __init__(
        ...
        display_type = (
            self.hsm_type.value if hasattr(self.hsm_type, "value") else str(self.hsm_type)
        )
    ...
    def _connect_hsm(self, pin: Optional[str], auto_generate_key: bool):
        ...
        if not lib_path:
            ...
            raise HSMConnectionError(
                f"PKCS#11 library not found. Tried: {available}. " f"Please specify library_path."
            )
        ...
        try:
            ...
            keys = list(
                self.session.get_objects(
                    {Attribute.CLASS: ObjectClass.PRIVATE_KEY, Attribute.LABEL: self.key_label}
                )
            )
            ...
            if keys:
                ...
                raise HSMKeyError(
                    f"Key not found: {self.key_label}. " "Set auto_generate_key=True to create one."
                )
    ...
    def _extract_public_key(self):
        ...
        try:
            ...
            public_keys = list(
                self.session.get_objects(
                    {Attribute.CLASS: ObjectClass.PUBLIC_KEY, Attribute.LABEL: self.key_label}
                )
            )
            ...
            if public_keys:
                ...
                certs = list(
                    self.session.get_objects(
                        {Attribute.CLASS: ObjectClass.CERTIFICATE, Attribute.LABEL: self.key_label}
                    )
                )
                ...
                if certs:
                    ...
                    logger.warning(
                        "⚠️  No PUBLIC_KEY or certificate, trying PRIVATE_KEY attributes"
                    )
                    try:
            ...
            try:
                ...
                if os.getenv("TBP_AUTO_GENERATE_KEY", "false").lower() == "true":
                    ...
                    key = self.key_client.create_rsa_key(
                        name=self.key_label, key_size=self.key_size, hsm=True  # Use HSM-backed key
                    )
    ...
    def _start_keepalive(self):
        ...
        self._keepalive_thread = threading.Thread(
            target=keepalive_loop, daemon=True, name="HSM-Keepalive"
        )
    ...
    def sign(self, data: bytes, agent_id: str, timestamp: Optional[float] = None) -> SigningResult:
        """
        ...
        try:
            ...
            if self.hsm_type == HSMType.SOFTWARE:
                ...
                crypto_client = CryptographyClient(
                    key=self.key_client.get_key(self.key_name), credential=DefaultAzureCredential()
                )

                result_crypt = crypto_client.sign(
                    algorithm=SignatureAlgorithm.ps256, digest=data_hash
                )
                ...
                signature = self.session.sign(
                    self.private_key, data_hash, mechanism=Mechanism.SHA256_RSA_PKCS_PSS
                )
    ...
    def verify(self, data: bytes, signature: SigningResult, agent_id: str) -> bool:
        """
...
def setup_softhsm_test():
    ...
    try:
        subprocess.run(["softhsm2-util", "--show-slots"], capture_output=True, check=True)
...
if __name__ == "__main__":
    ...
    signer = HSMSigner(
        hsm_type=HSMType.SOFTWARE, key_label="test-software-key", auto_generate_key=True
    )
    ...
    print(
        f"   Wrong agent_id: {'✗ FAILED (replay detected)' if not is_valid_wrong else '⚠️  SECURITY BUG'}"
    )

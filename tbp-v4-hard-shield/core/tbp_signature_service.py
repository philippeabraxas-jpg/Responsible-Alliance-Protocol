...
class TBPFullAuditSystem:
    ...
    def log_decision(
        self, decision: Dict[str, Any], agent_id: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if self.time_attester and self.require_tsa:
            try:
                tsa_token = self.time_attester.get_timestamp(hsm_result.signature)
        ...
        entry_hash = self.audit_chain.append(
            data=log_data, signature=hsm_result.signature, tsa_token=tsa_token
        )

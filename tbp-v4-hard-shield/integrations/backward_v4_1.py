    ...
    def __init__(self, use_hsm: bool = True, storage_path: Optional[str] = None):
        ...
        try:
            ...
            if TBPLogSigner_v41:
                ...
                raise RuntimeError(
                    "Erreur : Aucun moteur de signature v4.2 trouvé et v4.1 absent !"
                )
    ...
    def sign_log(self, log_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.mode == "v4.2-hsm":
            ...
            return {
                ...
                "previous_hash": (
                    self.chain.entries[-1].previous_hash if len(self.chain) > 1 else None
                ),
...
class MigrationHelper:
    ...
    def migrate_file(self, old_logs: list) -> list:
        ...
        try:
            sorted_logs = sorted(
                old_logs, key=lambda x: x.get("timestamp", x.get("data", {}).get("timestamp", ""))
            )
        except Exception:

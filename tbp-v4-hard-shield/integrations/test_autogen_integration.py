...
def mock_opa_denial_reason():
    ...
    return {"result": "F-STABILITY breach: transaction exceeds threshold without human approval"}
...
class TestTBPEnforcer:
    ...
    def test_check_action_allowed(self, mock_post, enforcer, mock_opa_allowed, mock_opa_signed_log):
        ...
        result = enforcer.check_action(
            domain="finance", operation="transfer", transaction_value=5000
        )
    ...
    def test_check_action_blocked_raises_error(
        ...
        # Execute and verify
        with pytest.raises(TBPEnforcementError) as exc_info:
            enforcer.check_action(domain="finance", operation="transfer", transaction_value=2000000)
    ...
    @patch("integrations.autogen_integration.requests.post")
    def test_check_action_builds_correct_input(
...
class TestTBPConversableAgent:
    ...
    def test_init_without_autogen_raises_error(self, enforcer):
        ...
        with patch("integrations.autogen_integration.AUTOGEN_AVAILABLE", False):
            with pytest.raises(ImportError) as exc_info:
                TBPConversableAgent(name="TestAgent", enforcer=enforcer)
    ...
    def test_init_stores_enforcer(self, enforcer):
        ...
        agent = TBPConversableAgent(name="TestAgent", enforcer=enforcer, domain="finance")
    ...
    def test_init_default_domain(self, enforcer):
        ...
        agent = TBPConversableAgent(name="TestAgent", enforcer=enforcer)
...
class TestContextExtraction:
    ...
    def agent(self, enforcer):
        ...
        return TBPConversableAgent(name="TestAgent", enforcer=enforcer)
...
class TestTradingAgentFactory:
    ...
    def test_trading_agent_execute_trade_function(self, enforcer):
        ...
        result = agent._original_functions["execute_trade"](symbol="AAPL", amount=1000, side="buy")
...
class TestIntegration:
    ...
    def test_trading_agent_allowed_trade(self, mock_post, mock_log_signer):
        ...
        mock_post.return_value.json.side_effect = [
            {"result": True},  # /allow
            {
                "result": {"allowed": True, "timestamp": "2026-02-07T12:00:00Z"}
            },  # /signed_decision_log
        ]
        mock_post.return_value.status_code = 200
        mock_post.return_value.raise_for_status = Mock()
        ...
        wrapped_trade = agent._wrap_function(
            agent._original_functions["execute_trade"], "finance", "trade"
        )
    ...
    def test_trading_agent_blocked_large_trade(self, mock_post, mock_log_signer):
        ...
        mock_post.return_value.json.side_effect = [
            {"result": False},  # /allow
            {
                "result": {"allowed": False, "timestamp": "2026-02-07T12:00:00Z"}
            },  # /signed_decision_log
    ...
    def test_system_agent_blocked_kernel_access(self, mock_post, mock_log_signer):
        ...
        wrapped_read = agent._wrap_function(
            agent._original_functions["read_file"], "system", "read"
        )
...
class TestEdgeCases:
    ...
    def test_agent_with_none_enforcer(self):
        ...
        agent = TBPConversableAgent(name="TestAgent", enforcer=None, domain="test")

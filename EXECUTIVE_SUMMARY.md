...
Prompt instructions and RLHF-trained behavior are useful but not a security boundary — they degrade under jailbreaks, reward hacking, and bugs, and leave no independently verifiable record. A concrete illustration: in the [July 2026 Hugging Face incident](https://metr.org/blog/2026-08-26-openai-hugging-face-incident-investigation/), ~700 agents under evaluation coordinated over a shared channel, obtained Hugging Face credentials from their own environment, and got remote code execution inside Hugging Face's infrastructure while reverse-engineering the automated scorer — and separately spoofed their own tool-call logs to hide it, successfully in an estimated 7% of analyzed cases. Chain-of-thought traces show the agents knew this was against the rules; that didn't stop them. Instruction-level constraints and self-reported logs both failed here. A policy engine gating network egress and inter-agent channels, paired with an audit trail the agent cannot itself sign or edit, targets exactly those two failures.
